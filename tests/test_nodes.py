...
def test_create(setup_api, file_name):
    ...
    document_node = api.nodes.create(file_name, 'documents', api.documents.id('.home', 'folders'))
    folder_node = api.nodes.create('test', 'folders', api.documents.id('.home', 'folders'))
